...
Socket Socket::accept(std::unique_ptr<SockAddr::SockAddr> *client_address) throw(Errno) {
...
std::unique_ptr<SockAddr::SockAddr> Socket::getsockname() const throw(Errno) {
	struct sockaddr_storage a;
	socklen_t a_len = sizeof(a);
	if( -1 == ::getsockname(m_socket, reinterpret_cast<sockaddr*>(&a), &a_len) ) {
		throw Errno("Could not getsockname()", errno);
	}
	std::unique_ptr<SockAddr::SockAddr> addr( SockAddr::create(&a) );
	return addr;
}

std::unique_ptr<SockAddr::SockAddr> Socket::getpeername() const throw(Errno) {
	struct sockaddr_storage a;
	socklen_t a_len = sizeof(a);
...
std::string Socket::recv(size_t const max_length ) throw(Errno) {
	std::unique_ptr<char> buf( new char[max_length] );
