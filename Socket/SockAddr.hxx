...
inline std::unique_ptr<SockAddr> create(struct sockaddr_in const *addr) throw(std::invalid_argument) {
	return create( reinterpret_cast<struct sockaddr_storage const*>(addr) );
}
inline std::unique_ptr<SockAddr> create(struct sockaddr_in6 const *addr) throw(std::invalid_argument) {
	return create( reinterpret_cast<struct sockaddr_storage const*>(addr) );
}

std::unique_ptr<SockAddr> translate(std::string const &host, unsigned short const port) throw(std::invalid_argument);

std::unique_ptr< boost::ptr_vector< SockAddr > > resolve(std::string const &host, std::string const &service, int const family = 0, int const socktype = 0, int const protocol = 0, bool const v4_mapped = false);
