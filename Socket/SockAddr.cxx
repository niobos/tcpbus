...
bool SockAddr::address_equal(SockAddr const &b) const throw() {
	if( this->addr_family() != b.addr_family() ) return false;
	switch( this->addr_family() ) {
	case AF_INET:
		return dynamic_cast<Inet4 const *>(this)->address_equal( dynamic_cast<Inet4 const &>(b) );
	case AF_INET6:
		return dynamic_cast<Inet6 const *>(this)->address_equal( dynamic_cast<Inet6 const &>(b) );
	}
}

std::unique_ptr<SockAddr> create(struct sockaddr_storage const *addr) throw(std::invalid_argument) {
	if( addr == NULL ) throw std::invalid_argument("Empty address");

	switch( addr->ss_family ) {
	case AF_INET:
		return std::unique_ptr<SockAddr>( new Inet4( *reinterpret_cast<const struct sockaddr_in*>(addr) ) );
	case AF_INET6:
		return std::unique_ptr<SockAddr>( new Inet6( *reinterpret_cast<const struct sockaddr_in6*>(addr) ) );
	default:
		throw(std::invalid_argument("Unknown address family"));
	}
}

std::unique_ptr<SockAddr> translate(std::string const &host, unsigned short const port) throw(std::invalid_argument) {
...
std::unique_ptr< boost::ptr_vector< SockAddr > > resolve(std::string const &host, std::string const &port, int const family, int const socktype, int const protocol, bool const v4_mapped) {
	...
	std::unique_ptr< boost::ptr_vector< SockAddr > > ret( new boost::ptr_vector< SockAddr> );
	while( p != NULL ) {
		std::unique_ptr<SockAddr> a( create( reinterpret_cast<sockaddr_storage*>(p->ai_addr) ) );
...
std::unique_ptr< boost::ptr_vector< SockAddr > > getifaddrs() {
	std::unique_ptr< boost::ptr_vector< SockAddr > > ret( new boost::ptr_vector< SockAddr> );
	...
	while( ifap != NULL ) {
		try {
			std::unique_ptr<SockAddr> a( create( reinterpret_cast<sockaddr_storage*>(ifap->ifa_addr) ) );
			ret->push_back( a.release() );
